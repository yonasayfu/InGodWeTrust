    ...
    <div class="py-6">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex items-center justify-between mb-6">
          <div>
            <h1 class="text-2xl font-semibold text-gray-900">Edit Target Donation</h1>
            <p class="mt-1 text-sm text-gray-600">Update target donation information.</p>
          </div>
          <div class="flex space-x-3">
            ...
            <Link
              :href="route('target-donations.show', targetDonation.id)"
              class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50"
            >
              View Details
            </Link>
          ...
          <form @submit.prevent="submit" class="p-6 space-y-6">
            ...
            <div class="flex justify-end pt-5 border-t border-gray-200">
              ...
              >
                ...
                <span v-else>Update Target Donation</span>
              </button>
            </div>
          </form>
        </div>
...
// Format the dates for the form inputs (YYYY-MM-DD format for date inputs)
...
const form = useForm({
  period: props.targetDonation.period,
  target_amount: props.targetDonation.target_amount,
  start_date: '',
  end_date: '',
});

onMounted(() => {
  // Set the dates after component is mounted
  form.start_date = formatDateForInput(props.targetDonation.start_date);
  form.end_date = formatDateForInput(props.targetDonation.end_date);
});

const submit = () => {
  form.put(route('target-donations.update', props.targetDonation.id));
};
</script>
